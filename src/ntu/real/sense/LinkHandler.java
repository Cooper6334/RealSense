...
class LinkHandler implements Runnable {
	...
	public void run() {
		...
		Log.i(Global.TAG, "+LinkHandler.run()");
		Log.i(Global.TAG, clientSocket.toString());
		...
		try {
			...
			Log.e(Global.TAG, "From client: " + userId);
			if (userId.equals("")) {
				userId = "�L�W��";
			}
			...
			mainHandler.sendMessage(mainHandler.obtainMessage(Global.ADD_CLIENT, userId));
			mainHandler.sendMessage(mainHandler.obtainMessage(Global.ADD_INPUT, input));
			mainHandler.sendMessage(mainHandler.obtainMessage(Global.ADD_OUTPUT, output));
